...
public class ClassPathScanningCandidateComponentProvider implements EnvironmentCapable, ResourceLoaderAware {
	...
	public Set<BeanDefinition> findCandidateComponents(String basePackage) {
		if (this.componentsIndex != null && indexSupportsIncludeFilters()) {
			...
			return scanCandidateComponents(basePackage); // 这里
		}
	}
	...
	private Set<BeanDefinition> scanCandidateComponents(String basePackage) {
		...
		try {
			...
			// 通过路径 拿到路径下所有的Class文件
			Resource[] resources = getResourcePatternResolver().getResources(packageSearchPath);
			boolean traceEnabled = logger.isTraceEnabled();
			boolean debugEnabled = logger.isDebugEnabled();
			// 对所有Class文件进行匹配 是否标注@Component注解
	...
	protected boolean isCandidateComponent(MetadataReader metadataReader) throws IOException {
		for (TypeFilter tf : this.excludeFilters) {
			if (tf.match(metadataReader, getMetadataReaderFactory())) { // 排除@Configuration注解的类 自动装配的类 启动类本身 注解上excludeFilters写的类
				return false;
			}
		}
		// includeFilters 里包含 @Component @ManagedBean @Named
		for (TypeFilter tf : this.includeFilters) {
			if (tf.match(metadataReader, getMetadataReaderFactory())) { // 两个类型匹配
