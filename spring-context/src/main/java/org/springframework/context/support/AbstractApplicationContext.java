...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 7.初始化MessageSource ,为上下文初始化Message源,即不同语言的消息体,国际化处理
				initMessageSource();
				...
				/* 11.初始化剩余非延迟加载单例Bean对象 */
				finishBeanFactoryInitialization(beanFactory);

				// Last step: publish corresponding event. 刷新完成后的动作
				// 12.完成容器的创建工作 , 完成刷新过程,通知生命周期处理器 同时发布事件通知别人
				finishRefresh();
			}
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// beanFactory 中注册 Environment 系统配置属性 系统环境信息
	...
	protected void initApplicationEventMulticaster() {
		...
		if (beanFactory.containsLocalBean(APPLICATION_EVENT_MULTICASTER_BEAN_NAME)) {
			...
			this.applicationEventMulticaster = new SimpleApplicationEventMulticaster(beanFactory); // 算是写死 ioc容器的事件广播器为SimpleApplicationEventMulticaster
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		/* 嵌入一个值解析器EmbeddedValueResolver 组件的注册,它负责解析占位符和表达式 */
		...
		/* 冻结 所有的definitionMap 不变 */
		beanFactory.freezeConfiguration();

		// Instantiate all remaining (non-lazy-init) singletons.
		/* 真正->初始化剩余非延迟加载单例Bean对象 */
